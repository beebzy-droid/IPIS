...
- **2026-06-03** — 1C framing decided (A+C; SBC inapplicable to literal
  ...
  Resume = single-mode baseline → 3-method SBC migration + data-fraction sweep.HEADLINE (generated data, source=mode1, theta=2, lam=0.3, n_repeats=8, GP
subsample ~350, two-layer = migration offline + 1B bias-update online):
ROBUST METRIC = data efficiency to reach 90% of the from-scratch ceiling (the
strict "exceed the from-scratch *ceiling*" crossover is BRITTLE here because the
ceilings coincide -- it flipped 20%<->100% across runs/seeds; do NOT headline it).
- **mode2 (G=58): ~10x data efficiency** -- migration reaches 90% of from-scratch
  full accuracy (R2~0.40) at ~5% target data vs ~50% from-scratch. Coverage 92-98%.
- **mode3 (G=63, LARGE gap): ~10x data efficiency** -- same, R2~0.37 target at ~5%
  vs ~50%. Coverage 94-98%, intervals WIDER (2.8-4.0 vs mode2 2.6-3.2) -- uncertainty
  scales with the gap. Robust low-data DOMINANCE both modes (migrated >> from-scratch
  at every low fraction; mode3 @5%: +0.43 vs from-scratch -0.08).
=> Migration + online bias-update is ~10x more data-efficient than from-scratch on
a new operating regime (>=10x; 5% is our grid floor, true efficiency may be higher;
5% estimates have wide error bars +/-0.11). MAPIE/conformal DISCHARGED (calibrated
95% GP-posterior intervals on the transferred model, widening with the gap).
CAVEAT: report data-efficiency (robust), not the knife-edge crossover. Reproduce
with --n-repeats 8 (n_repeats=3 is too noisy at low fractions).
