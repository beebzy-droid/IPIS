...
| Phase | Scope | Status |
|---|---|---|
| 2A — Health index + anomaly detection (FEMTO + CWRU) | Vibration feature pipeline, physics-anchored fault-frequency features, health index, ADWIN/SPC alarming | ◐ In progress — physics layer (gate 0.012%), CWRU loader (real schema), and vibration features (time-domain + squared-envelope + fault-band ratios; synthetic IR validation 0.1%) landed. Remaining: health index + real-data validation run |
