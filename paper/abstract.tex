...
present an integrated framework that addresses each with a simple,
...
residuals, and functional scale-bias migration composed with the online layer. The framework is completed by an
online implementation that scores each delayed laboratory result against the interval
issued when its sample was predicted. On the Tennessee Eastman process the framework delivers
regime-uniform empirical coverage of 0.897--0.903 against a 0.90 target where a static
conformal baseline swings between 0.847 and 0.957, and 10.0$\times$ data efficiency in regime
migration. A deliberately hostile negative control (SECOM: 590 anonymized features,
no physics) shows the decomposition: the point sensor fails ($R^2$ = --1.84), yet coverage
