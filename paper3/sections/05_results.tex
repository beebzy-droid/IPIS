...
the relative interval back-off (right axis) degrade monotonically. The actionable band ($b<0.5$)
ends at $\eta=0.68$ and degeneracy ($b\ge1$) begins at $\eta=1.97$, where the certified lower bound
is no longer positive on average.}\label{fig:dep}
\end{figure}
...
here. Appendix~\ref{app:bearing} works that identification through for rolling-contact fatigue,
where ISO~281 enumerates the residual groups directly, and the contrast with the gas path is the
clearest statement we can give of where the certificate applies. Where $\boldsymbol\psi$ is unavailable the runtime diagnostic of
...
conditions (derived for these conditions in Appendix~\ref{app:bearing}). The diagnostic therefore returns \emph{indeterminate} and SCC declines its a-priori
